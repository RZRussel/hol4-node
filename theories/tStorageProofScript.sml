...
load "preamble";
load "basis";
load "basisProgTheory";
load "ml_monad_translator_interfaceLib";
load "StorageProgTheory";
load "StorageFFITheory";
*)

open preamble basis basisProgTheory ml_monad_translator_interfaceLib;
open tStorageProgTheory tStorageFFITheory;
open integerTheory cfDivTheory cfDivLib;
open cfMonadTheory;

val _ = temp_delsimps ["NORMEQ_CONV"];

val _ = new_theory "tStorageProof";

val _ = translation_extends"StorageProg";
val _ = option_monadsyntax.temp_add_option_monadsyntax();

val IODBFS_def = Define `
  IODBFS dbfs = IOx (db_ffi_part) dbfs`;
...
Theorem dbopen_spec:
   \<forall>s sv fs.
     FILENAME s sv \<and>
     hasFreeFD fs \<Rightarrow>
     app (p:'ffi ffi_proj) Storage_db_open_v [sv]
       (IODBFS dbfs)
       (POSTve
          (\fdv. &(INSTREAM (nextFD fs) fdv \<and>
                  validFD (nextFD fs) (openFileFS s fs ReadMode 0) \<and>
                  inFS_fname fs s) * IODBFS dbfs)
          (\e. &(StorageFailed_exn e) * IODBFS dbfs))
Theorem UNIQUE_IODBFS:
!s. VALID_HEAP s ==> !fs1 fs2 H1 H2. (IODBFS fs1 * H1) s /\
                                      (IODBFS fs2 * H2) s ==> fs1 = fs2
Proof
  rw[IODBFS_def,cfHeapsBaseTheory.IOx_def, db_ffi_part_def,
...
val FILENAME_def = Define `
  FILENAME s sv =
    (STRING_TYPE s sv \<and>
     \<not>MEM (CHR 0) (explode s) \<and>
     strlen s < 256 * 256)
`;
...
Theorem STRING_FILENAME_UNICITY_R[xlet_auto_match]:
  !f fv fv'. STRING_TYPE f fv ==>
    (FILENAME f fv' <=> fv' = fv /\ \<not>MEM #"\^@" (explode f) /\ strlen f < 256 * 256)
Proof
  filename_tac
QED;
...
Theorem STRING_FILENAME_UNICITY_L[xlet_auto_match]:
  !f f' fv. STRING_TYPE f fv ==>
    (FILENAME f' fv <=> f' = f /\ \<not>MEM #"\^@" (explode f) /\ strlen f < 256 * 256)
Proof
  filename_tac
QED;
...
Theorem stdo_numchars:
   stdo fd name (fs with numchars := l) out \<Leftrightarrow> stdo fd name fs out
Proof
  rw[stdo_def]
QED
...
Theorem stdo_add_stdo:
   stdo fd nm fs init \<Rightarrow> stdo fd nm (add_stdo fd nm fs out) (strcat init out)
Proof
  rw[add_stdo_def]
...
Theorem add_stdo_nil:
   stdo fd nm fs out \<Rightarrow> add_stdo fd nm fs (strlit "") = fs
Proof
  rw[add_stdo_def]
  \\ SELECT_ELIM_TAC
...
Theorem STD_streams_stdout:
   STD_streams fs \<Rightarrow> \<exists>out. stdout fs out
Proof
  rw[STD_streams_def,stdo_def] \\ rw[] \\ metis_tac[explode_implode,strlen_implode]
QED

Theorem STD_streams_stderr:
   STD_streams fs \<Rightarrow> \<exists>out. stderr fs out
Proof
  rw[STD_streams_def,stdo_def] \\ rw[] \\ metis_tac[explode_implode,strlen_implode]
QED

Theorem STD_streams_add_stdout:
   STD_streams fs \<Rightarrow> STD_streams (add_stdout fs out)
Proof
  rw[]
...
Theorem STD_streams_add_stderr:
   STD_streams fs \<Rightarrow> STD_streams (add_stderr fs out)
Proof
  rw[]
...
Theorem validFD_up_stdo[simp]:
   validFD fd (up_stdo fd' fs out) \<Leftrightarrow> validFD fd fs
Proof
  rw[up_stdo_def]
QED

Theorem validFD_add_stdo[simp]:
   validFD fd (add_stdo fd' nm fs out) \<Leftrightarrow> validFD fd fs
Proof
  rw[add_stdo_def]
QED

Theorem validFileFD_up_stdo[simp]:
   validFileFD fd (up_stdo fd' fs out).infds \<Leftrightarrow> validFileFD fd fs.infds
Proof
  rw[up_stdo_def]
QED

Theorem validFileFD_add_stdo[simp]:
   validFileFD fd (add_stdo fd' nm fs out).infds \<Leftrightarrow> validFileFD fd fs.infds
Proof
  rw[add_stdo_def]
QED

Theorem up_stdo_ADELKEY:
   fd \<noteq> fd' \<Rightarrow>
...
Theorem get_file_content_add_stdout:
   STD_streams fs \<and> fd \<noteq> 1 \<Rightarrow>
...
Theorem get_file_content_add_stderr:
   STD_streams fs \<and> fd \<noteq> 2 \<Rightarrow>
...
Theorem up_stdo_forwardFD:
   fd \<noteq> fd' \<Rightarrow> up_stdo fd' (forwardFD fs fd n) out = forwardFD (up_stdo fd' fs out) fd n
Proof
...
Theorem stdo_forwardFD:
   fd \<noteq> fd' \<Rightarrow> (stdo fd' nm (forwardFD fs fd n) out \<Leftrightarrow> stdo fd' nm fs out)
Proof
...
Theorem stdo_fastForwardFD:
   fd \<noteq> fd' \<Rightarrow> (stdo fd' nm (fastForwardFD fs fd) out \<Leftrightarrow> stdo fd' nm fs out)
Proof
...
Theorem add_stdo_forwardFD:
   fd \<noteq> fd' \<Rightarrow> add_stdo fd' nm (forwardFD fs fd n) out = forwardFD (add_stdo fd' nm fs out) fd n
Proof
...
Theorem stdin_11:
   stdin fs i1 p1 \<and> stdin fs i2 p2 \<Rightarrow> i1 = i2 \<and> p1 = p2
Proof
  rw[stdin_def] \\ fs[]
QED

Theorem stdin_get_file_content:
   stdin fs inp pos \<Rightarrow> get_file_content fs 0 = SOME (inp,pos)
Proof
  rw[stdin_def,fsFFITheory.get_file_content_def]
QED

Theorem stdin_forwardFD:
   stdin fs inp pos \<Rightarrow>
...
Theorem stdin_get_stdin:
   stdin fs inp pos \<Rightarrow> get_stdin fs = DROP pos inp
Proof
  rw[get_stdin_def]
  \\ SELECT_ELIM_TAC
...
val FD_def = Define `
  FD fd fdv = (STRING_TYPE (strlit(MAP (CHR \<circ> w2n) (n2w8 fd))) fdv \<and> fd < 256**8)`;

val INSTREAM_def = Define `
  INSTREAM fd fdv <=>
    INSTREAM_TYPE (Instream (strlit(MAP (CHR \<circ> w2n) (n2w8 fd)))) fdv \<and>
    fd < 256**8`

val OUTSTREAM_def = Define `
  OUTSTREAM fd fdv <=>
    OUTSTREAM_TYPE (Outstream (strlit(MAP (CHR \<circ> w2n) (n2w8 fd)))) fdv \<and>
    fd < 256**8`
...
Theorem openIn_spec:
   \<forall>s sv fs.
     FILENAME s sv \<and>
     hasFreeFD fs \<Rightarrow>
     app (p:'ffi ffi_proj) dbIO_dbopen_v [sv]
       (IOFS fs)
       (POSTve
          (\fdv. &(INSTREAM (nextFD fs) fdv \<and>
                  validFD (nextFD fs) (openFileFS s fs ReadMode 0) \<and>
          ...
          (\e. &(BadFileName_exn e \<and> ~inFS_fname fs s) * IOFS fs))
Proof
   ...
   \<forall>s sv fs.
     ...
     app (p:'ffi ffi_proj) TextIO_openIn_v [sv]
       ...
       (POSTve
          ...
          (\e. &(BadFileName_exn e \<and> ~inFS_fname fs s) * STDIO fs))
Proof
...
Theorem closeIn_STDIO_spec:
   \<forall>fd fs fdv.
     INSTREAM fd fdv /\ fd >= 3 /\ fd <= fs.maxFD \<Rightarrow>
     app (p:'ffi ffi_proj) TextIO_closeIn_v [fdv]
       (STDIO fs)
       (POSTve
          (\u. &(UNIT_TYPE () u /\ validFileFD fd fs.infds) *
               STDIO (fs with infds updated_by ADELKEY fd))
          (\e. &(InvalidFD_exn e /\ \<not> validFileFD fd fs.infds) * STDIO fs))
Proof
...
Theorem closeOut_STDIO_spec:
   \<forall>fd fs fdv.
     OUTSTREAM fd fdv /\ fd >= 3 /\ fd <= fs.maxFD \<Rightarrow>
     app (p:'ffi ffi_proj) TextIO_closeOut_v [fdv]
       (STDIO fs)
       (POSTve
          (\u. &(UNIT_TYPE () u /\ validFileFD fd fs.infds) *
...
Theorem writei_spec:
  wfFS fs \<Rightarrow> 0 < n \<Rightarrow>
   MAX (i+n) 2048 <= LENGTH rest \<Rightarrow> i + n < 256**2 \<Rightarrow>
  get_file_content fs fd = SOME(content, pos) \<Rightarrow>
  get_mode fs fd = SOME WriteMode \<Rightarrow>
  FD fd fdv \<Rightarrow> NUM n nv \<Rightarrow> NUM i iv \<Rightarrow>
  bc = h1 :: h2 :: h3 :: h4 :: rest \<Rightarrow>
...
Proof
  ...
  `ll \<noteq> [||]`  by (cases_on`ll` >> fs[wfFS_def,liveFS_def,live_numchars_def]) >>
  `always (eventually (\<lambda>ll. \<exists>k. LHD ll = SOME k \<and> k \<noteq> 0)) ll`
...
Theorem write_spec:
   ...
   wfFS fs \<Rightarrow> MAX(i + n) 2048 <= LENGTH rest \<Rightarrow> i + n < 256 ** 2 \<Rightarrow>
   get_file_content fs fd = SOME(content, pos) \<Rightarrow>
   get_mode fs fd = SOME WriteMode \<Rightarrow>
   FD fd fdv \<Rightarrow> NUM n nv \<Rightarrow> NUM i iv \<Rightarrow>
   bc = h1 :: h2 :: h3 :: h4 :: rest \<Rightarrow>
...
Proof
  ...
  qexists_tac`insert_atI (TAKE nw (MAP (CHR \<circ> w2n) (DROP i rest))) pos content` >>
  NTAC 3 (strip_tac >-(
      imp_res_tac get_file_content_validFD >>
...
Theorem output1_spec:
   !fd fdv c cv bc content pos.
    get_file_content fs fd = SOME(content, pos) \<Rightarrow>
    get_mode fs fd = SOME WriteMode \<Rightarrow>
    CHAR c cv \<Rightarrow> OUTSTREAM fd fdv \<Rightarrow>
...
Theorem output1_STDIO_spec:
   !fd. get_file_content fs fd = SOME(content, pos) \<and>
        get_mode fs fd = SOME WriteMode \<and>
   CHAR c cv \<and> OUTSTREAM fd fdv \<Rightarrow>
   app (p:'ffi ffi_proj) TextIO_output1_v [fdv; cv]
   (STDIO fs)
   (POSTv uv.
     &UNIT_TYPE () uv *
...
Theorem output_spec:
   !s fd fdv sv fs content pos.
    OUTSTREAM fd fdv \<Rightarrow> STRING_TYPE s sv \<Rightarrow>
    (get_file_content fs fd = SOME(content, pos)) \<Rightarrow>
    (get_mode fs fd = SOME WriteMode) \<Rightarrow>
...
Theorem output_STDIO_spec:
   ...
   OUTSTREAM fd fdv \<and> get_file_content fs fd = SOME (content,pos) \<and> get_mode fs fd = SOME WriteMode \<and> STRING_TYPE s sv \<Rightarrow>
   app (p:'ffi ffi_proj) TextIO_output_v [fdv;sv]
   (STDIO fs)
   (POSTv uv. &(UNIT_TYPE () uv) *
...
Proof
  ...
  \\ pairarg_tac \\ fs[] \\ rveq
  \\ `fd = 1 \<or> fd = 2 \<Rightarrow> off = LENGTH content`
  by (
    fs[STD_streams_def]
...
Theorem output_stderr_spec:
   !fs sv s fdv.
    STRING_TYPE s sv \<and> fdv = stderr_v \<Rightarrow>
    app (p:'ffi ffi_proj) TextIO_output_v [fdv;sv]
    (STDIO fs)
...
Theorem read_spec:
  !fs fd fdv n nv. wfFS fs \<Rightarrow> FD fd fdv \<Rightarrow> NUM n nv \<Rightarrow>
   n < 256**2 \<Rightarrow> MAX n 2048 <= LENGTH rest \<Rightarrow>
   ...
   (POSTve
     (\nrv. SEP_EXISTS (nr : num).
      ...
      SEP_EXISTS content pos.
        &(get_file_content fs fd = SOME(content, pos) /\
          ...
          (nr = 0 \<Leftrightarrow> eof fd fs = SOME T \<or> n = 0)) *
     ...
     (\e. &InvalidFD_exn e * &(get_file_content fs fd = NONE \<or> get_mode fs fd \<noteq> SOME ReadMode) * IOFS fs))
Proof
...
Theorem read_into_spec:
  !fs fd fdv n nv buf. wfFS fs \<Rightarrow> FD fd fdv \<Rightarrow> NUM n nv \<Rightarrow>
   n < 256**2 \<Rightarrow> MAX n 1024 <= LENGTH rest \<Rightarrow>
   ...
   (POSTve
     (\nrv. SEP_EXISTS (nr : num).
      ...
      W8ARRAY buf (0w :: n2w (nr DIV 256) :: n2w nr :: h4::
        MAP (n2w o ORD) (TAKE nr (DROP pos content))++DROP nr rest))
     (\e. &InvalidFD_exn e * &(get_file_content fs fd = NONE \<or> get_mode fs fd \<noteq> SOME ReadMode) * IOFS_WO_iobuff fs))
Proof
...
Theorem read_byte_spec:
   !fd fdv content pos.
    FD fd fdv \<Rightarrow>
    get_file_content fs fd = SOME(content, pos) \<Rightarrow>
    get_mode fs fd = SOME ReadMode \<Rightarrow>
...
Theorem read_byte_STDIO_spec:
    FD fd fdv \<and> fd \<noteq> 1 \<and> fd \<noteq> 2 \<and>
    get_file_content fs fd = SOME(content, pos) \<Rightarrow>
    get_mode fs fd = SOME ReadMode \<Rightarrow>
    app (p:'ffi ffi_proj) TextIO_read_byte_v [fdv]
    (STDIO fs)
...
Theorem input1_spec:
  INSTREAM fd fdv \<and> fd \<noteq> 1 \<and> fd \<noteq> 2 \<and>
  get_file_content fs fd = SOME(content, pos) \<Rightarrow>
  get_mode fs fd = SOME ReadMode \<Rightarrow>
...
Proof
  ...
  \\ xhandle`POSTve (\<lambda>v. &OPTION_TYPE CHAR (SOME (EL pos content)) v *
                         STDIO (forwardFD fs fd 1) * &(eof fd fs = SOME F))
                    (\<lambda>e. &EndOfFile_exn e * STDIO fs * &(eof fd fs = SOME T))`
  >- (
    fs [INSTREAM_def]
    \\ xlet_auto >- xsimpl \\ fs [get_in_def]
...
Theorem input_IOFS_spec:
  !fd fdv fs content pos off offv.
    len + off <= LENGTH buf \<and>
    INSTREAM fd fdv \<and> NUM off offv \<and> NUM len lenv \<and>
    get_file_content fs fd = SOME(content, pos) \<Rightarrow>
    get_mode fs fd = SOME ReadMode \<Rightarrow>
...
Proof
  ...
  reverse(Cases_on`pos \<le> LENGTH content`) >- (
    imp_res_tac get_file_content_eof \\ rfs[] \\
    reverse(Cases_on`wfFS fs`) >- (fs[IOFS_def] \\ xpull) \\
    simp[MAX_DEF,MIN_DEF] \\
    xfun_spec`input0`
    `\<forall>offv lenv countv.
     NUM len lenv \<and> NUM 0 countv \<Rightarrow>
 ...
 xfun_spec`input0`
  `!count countv buf fs pos off offv lenv.
    len + off <= LENGTH buf \<Rightarrow> pos <= LENGTH content  \<Rightarrow> NUM count countv \<Rightarrow>
    INSTREAM fd fdv \<Rightarrow> NUM off offv \<Rightarrow> NUM len lenv \<Rightarrow>
    get_file_content fs fd = SOME(content, pos) \<Rightarrow>
    get_mode fs fd = SOME ReadMode \<Rightarrow>
...
Proof
  rw[STDIO_def]
  \\ xpull
  \\ `fd = 1 \<or> fd = 2 \<Rightarrow> pos = LENGTH content`
  by (
    fs[STD_streams_def]
    \\ fs[get_file_content_def]
    \\ pairarg_tac \\ fs[]
    ...
    \\ rw[] \\ fs[]
    \\ metis_tac[SOME_11] )
  \\ `pos = LENGTH content \<Rightarrow> MIN (len + pos) (MAX pos (LENGTH content)) = LENGTH content` by simp[MAX_DEF,MIN_DEF]
...
Theorem b_openInSetBufferSize_spec:
  \<forall>s sv fs bsize bsizev bactive.
     FILENAME s sv \<and>
     NUM bsize bsizev /\
     hasFreeFD fs \<Rightarrow>
     app (p:'ffi ffi_proj) TextIO_b_openInSetBufferSize_v [sv;bsizev]
       ...
       (POSTve
          (\is. &(
                ...
                IOFS (openFileFS s fs ReadMode 0))
          (\e. &(BadFileName_exn e \<and> ~inFS_fname fs s)
                   * IOFS fs))
Proof
...
Theorem b_openIn_STDIO_spec:
   \<forall>s sv fs.
     ...
     app (p:'ffi ffi_proj) TextIO_b_openIn_v [sv]
       (STDIO fs)
       (POSTve
          (\is. &(validFD (nextFD fs) (openFileFS s fs ReadMode 0) \<and>
                  inFS_fname fs s) *
                INSTREAM_BUFFERED_FD [] (nextFD fs) is *
                STDIO (openFileFS s fs ReadMode 0))
          (\e. &(BadFileName_exn e \<and> ~inFS_fname fs s) * STDIO fs))
   ...
   \<forall>fd fs.
     app (p:'ffi ffi_proj) TextIO_b_closeIn_v [is]
       ...
       (POSTve
          ...
          (\e. &(InvalidFD_exn e /\ \<not> validFileFD fd fs.infds) * IOFS fs))
Proof
...
Theorem b_refillBuffer_with_read_spec:
  ...
  (POSTv wv. SEP_EXISTS (nr:num) h4 rest.
                 &(NUM nr wv /\
                   ...
                   (nr = 0 \<Leftrightarrow> eof fd fs = SOME T) /\
                   (nr = 0 \<Leftrightarrow> ~(pos < STRLEN content)) /\
...
Proof
    ...
    >-(xlet `POSTv dc.
              SEP_EXISTS (nr:num) h4 rest.
                &((nr = 0 \<Leftrightarrow> STRLEN content <= pos) /\
      ...
      >-(xsimpl \\ rpt strip_tac \\ fs[instream_buffered_inv_def]
        ...
        \\ conj_tac
        >-(qabbrev_tac `bytes = (DROP pos (MAP (n2w \<circ> ORD) content)) : word8 list`
          ...
          \\ `0 + pos < LENGTH (MAP (n2w \<circ> ORD) content)` by fs[Abbr`bytes`]
          \\ `h = EL pos (MAP (n2w \<circ> ORD) content)` by fs[Abbr`bytes`, EL_DROP]
        ...
        >-(simp[SEG_TAKE_DROP]
          \\ fs [GSYM NOT_LESS]
          \\ qpat_x_assum `TAKE (w' − r') (DROP pos (MAP (n2w \<circ> ORD) content)) = _ :: _`
                 assume_tac
...
Theorem dropUntil_take_isPrefix:
  !P l x.
      dropUntil P (TAKE x l) \<preceq> dropUntil P l
Proof
  strip_tac
  \\ completeInduct_on `LENGTH l`
  \\ rpt strip_tac \\ rveq \\ fs [PULL_FORALL]
...
Theorem dropUntilIncl_take_isPrefix:
  !P l x.
      dropUntilIncl P (TAKE x l) \<preceq> dropUntilIncl P l
Proof
...
Proof
  ...
  \\ Cases_on `l`
  >-(fs[EXISTS_DEF])
  >-(Cases_on `($= ((n2w:num->word8 \<circ> ORD) x)) h`
    >-(fs[EXISTS_DEF]
      \\ disj1_tac
...
Theorem exists_eq_o_map2:
    !P l.
      EXISTS ($= ((n2w \<circ> ORD) x)) (MAP (n2w:num->word8 o ORD) l) <=>
        EXISTS ($= x) (l:string)
Proof
  strip_tac
  \\ completeInduct_on `LENGTH (l:string)`
...
Theorem map_w82c_takeUntilIncl_eq_takeUntilIncl_map_c2w8:
  !P l.
      takeUntilIncl ($= c) (MAP (CHR \<circ> w2n) (l:word8 list)) =
...
Theorem map_w82c_dropUntilIncl_eq_dropUntilIncl_map_c2w8:
  !P l.
      dropUntilIncl ($= c) (MAP (CHR \<circ> w2n) (l:word8 list)) =
...
Theorem LENGTH_takeUntilIncl_w8_takeUntilIncl_chr:
  !ls.
    LENGTH (takeUntilIncl ($= (10w:word8)) (MAP (n2w \<circ> ORD) (DROP pos content))) =
...
Theorem isPREFIX_FILTER:
  !ls bs. ls \<preceq> bs ==> FILTER P ls \<preceq> FILTER P bs
Proof
  strip_tac
  \\ completeInduct_on `LENGTH ls`
  \\ rpt strip_tac \\ rveq \\ fs [PULL_FORALL]
...
Theorem LENGTH_isPREFIX:
  !ls bs. ls \<preceq> bs ==> LENGTH ls <= LENGTH bs
Proof
  strip_tac
  \\ completeInduct_on `LENGTH ls`
  \\ rpt strip_tac \\ rveq \\ fs [PULL_FORALL]
...
Proof
  ...
  \\ `FILTER P (h::t) \<preceq> (TL (FILTER P bs))` by fs[]
...
Theorem isPREFIX_EQ_LENGTH:
  !ls rs.
    (LENGTH ls = LENGTH rs  /\ ls \<preceq> rs) <=>
      ls = rs
Proof
    completeInduct_on `LENGTH ls`
    \\ rpt strip_tac \\ rveq \\ fs [PULL_FORALL]
...
Proof
  ...
  >-(`($= #"\n") (LAST (STRCAT ls xs))` by fs[]
    ...
    \\ qpat_x_assum `STRCAT ls xs \<noteq> ""` kall_tac
    \\ qpat_x_assum `STRCAT rs xs \<noteq> ""` kall_tac
    \\ `rs <> []` by fs[Abbr`rs`]
  ...
  >-(`~($= #"\n") (LAST (STRCAT ls xs))` by fs[]
    ...
    \\ qpat_x_assum `LAST (STRCAT ls xs) = LAST xs` kall_tac
    \\ qpat_x_assum `LAST (STRCAT rs xs) = LAST xs` kall_tac
    \\ qpat_x_assum `#"\n" \<noteq> LAST xs` kall_tac
    \\ qpat_x_assum `#"\n" \<noteq> LAST (STRCAT ls xs)` kall_tac
    \\ qpat_x_assum `#"\n" \<noteq> LAST (STRCAT rs xs)` kall_tac
    \\ qpat_x_assum `STRCAT ls xs \<noteq> ""` kall_tac
    \\ qpat_x_assum `STRCAT rs xs \<noteq> ""` kall_tac
...
Theorem isPREFIX_TAKE_LENGTH:
  !ls rs.
    ls \<preceq> rs <=> (ls = TAKE (LENGTH ls) rs /\ LENGTH ls <= LENGTH rs)
Proof
  completeInduct_on `LENGTH ls`
  \\ rpt strip_tac \\ rveq \\ fs [PULL_FORALL]
  \\ eq_tac
...
Theorem isPREFIX_LENGTH_LEQ:
  !ls rs.
    ls \<preceq> rs ==> LENGTH ls <= LENGTH rs
Proof
  completeInduct_on `LENGTH ls`
  \\ rpt strip_tac \\ rveq \\ fs [PULL_FORALL]
  \\ cases_on `ls` >- fs[]
...
Proof
  ...
  \\ `SUC (LENGTH t) + 1 \<le> LENGTH (dropUntil P t')` by decide_tac
...
Theorem isPREFIX_MAP:
  !ls rs.
    ls \<preceq> rs ==> MAP f ls \<preceq> MAP f rs
Proof
  completeInduct_on `LENGTH ls`
  \\ rpt strip_tac \\ rveq \\ fs [PULL_FORALL]
  \\ cases_on `ls` >- fs[]
...
Theorem forwardFD_b_lineForwardFD_not_in_buffer:
  !buff fs fd extra.
    get_file_content fs fd = SOME (content, pos) /\
    (extra \<preceq> dropUntilIncl ($= (10w:word8)) (MAP (n2w:num->word8 o ORD) (DROP pos content)) \/
     extra = []) /\
...
Proof
  ...
  >-(Cases_on `bactive` >-fs[]
    ...
    >-(xlet_auto >- (xcon \\ xsimpl) \\ simp[INSTREAM_BUFFERED_FD_def] \\ xpull
      ...
      \\ `(n2w:num->word8 \<circ> ORD) ch = h` by fs[] \\ simp[]
      \\ reverse (Cases_on `n2w:num->word8 (ORD ch) = h \<or> EXISTS ($= (n2w (ORD ch))) leftover`)
   ...
   (simp[]
    ...
    >-
     (xlet `POSTv av. SEP_EXISTS leftover.
       ...
       (reverse (Cases_on `EXISTS ($= (n2w (ORD ch))) leftover`)
        THEN1
         (xlet `POSTv dv. SEP_EXISTS leftover2.
           ...
           (simp[INSTREAM_BUFFERED_FD_def] \\ xpull
            ...
            \\ `LENGTH
                (dropUntilIncl ($= (n2w:num->word8 (ORD ch)))
                    (DROP (pos + (LENGTH leftover + 1)) (MAP (n2w \<circ> ORD) content))) <=
                LENGTH (DROP (pos + (LENGTH leftover + 1)) (MAP (n2w:num->word8 \<circ> ORD) content))`
                by fs[LENGTH_dropUntilIncl]
            \\ fs[LENGTH_DROP]
            \\ `LENGTH (takeUntilIncl ($= (n2w:num->word8 (ORD ch)))
                (DROP (pos + (LENGTH leftover + 1)) (MAP (n2w:num->word8  \<circ> ORD) content))) <=
                LENGTH (DROP (pos + (LENGTH leftover + 1)) (MAP (n2w:num->word8  \<circ> ORD) content))`
          ...
          \\ simp[]
          \\ fs[explode_fromI_def, take_fromI_def]
          \\ `pos < LENGTH (MAP (n2w:num->word8 \<circ> ORD) content)` by fs[NOT_LESS, LENGTH_MAP]
          \\ `pos < LENGTH content` by fs[]
          \\ imp_res_tac take_drop_eq_hd_cons
          \\ pop_assum (qspecl_then [`LENGTH leftover`] mp_tac)
          \\ strip_tac \\ `EL pos (MAP (n2w:num->word8 \<circ> ORD) content) =
                                HD (DROP pos (MAP (n2w:num->word8 \<circ> ORD) content))`
                                   by fs[HD_DROP] \\ rw[]
          \\ imp_res_tac chr_neq_ord_o_w8_neq
          \\ `($~ \<circ> $= (n2w:num->word8 (ORD ch))) ((n2w o ORD) (EL pos content))` by fs[]
          \\ imp_res_tac (INST_TYPE [beta|->``:word8``] EL_MAP) \\ pop_assum kall_tac
          \\ pop_assum (qspecl_then [`(n2w o ORD)`] mp_tac) \\ strip_tac
          \\ imp_res_tac HD_DROP
          \\ `TAKE (LENGTH leftover) (DROP (pos + 1) (MAP (n2w:num->word8 \<circ> ORD) content)) =
                 leftover` by fs[]
          \\ `EVERY ($~ \<circ> $= (n2w:num->word8 (ORD ch)))
                    (HD (DROP pos (MAP (n2w:num->word8 \<circ> ORD) content))::
                      TAKE (LENGTH leftover)
                        (DROP (pos + 1) (MAP (n2w:num->word8 \<circ> ORD) content)))`
                  by fs[EVERY_DEF]
          \\ `TAKE (LENGTH leftover + 1) (DROP pos (MAP (n2w:num->word8 \<circ> ORD) content)) =
                  (HD (DROP pos (MAP (n2w:num->word8 \<circ> ORD) content))::
                      TAKE (LENGTH leftover)
                        (DROP (pos + 1) (MAP (n2w:num->word8 \<circ> ORD) content)))` by fs[]
          \\ `EVERY ($~ \<circ> $= (n2w:num->word8 (ORD ch)))
                      (TAKE (LENGTH leftover + 1)
                        (DROP (pos) (MAP (n2w:num->word8 \<circ> ORD) content)))`
                  by fs[]
          \\ rpt conj_tac
          THEN1
           (pop_assum mp_tac \\ ntac 8 (pop_assum kall_tac) \\ strip_tac
            \\ Cases_on `DROP pos (MAP (n2w:num->word8 \<circ> ORD) content)`
            THEN1(fs[DROP_NIL])
            THEN1
             (fs[HD]
              \\ simp[takeUntilIncl_cons]
          ...
          THEN1
           (xsimpl \\ imp_res_tac takeUntilIncl_length_drop \\ pop_assum mp_tac
            ...
            \\ Cases_on `LENGTH leftover + 1 <=
                         LENGTH (DROP pos (MAP (n2w:num->word8 \<circ> ORD) content))`
             ...
             (fs[explode_fromI_def, take_fromI_def] \\ rfs[] \\ pop_assum mp_tac
              \\ qpat_x_assum `
                  EXISTS ($= (n2w (ORD ch))) (TAKE (w' − r')
                    (DROP (pos + 1) (MAP (n2w \<circ> ORD) content)))` mp_tac
        ...
        \\ strip_tac \\ `EL pos (MAP (n2w:num->word8 \<circ> ORD) content) =
                            HD (DROP pos (MAP (n2w:num->word8 \<circ> ORD) content))`
                               by fs[HD_DROP]
        \\ imp_res_tac chr_neq_ord_o_w8_neq
        \\ `($~ \<circ> $= (n2w:num->word8 (ORD ch))) ((n2w o ORD) (EL pos content))`
                by fs[]
        \\ imp_res_tac (INST_TYPE [beta|->``:word8``] EL_MAP) \\ pop_assum kall_tac
        \\ pop_assum (qspecl_then [`(n2w o ORD)`] mp_tac) \\ strip_tac
        \\ imp_res_tac HD_DROP
        ...
        \\ `TAKE (LENGTH leftover + 1) (DROP pos (MAP (n2w:num->word8 \<circ> ORD) content)) =
                (HD (DROP pos (MAP (n2w:num->word8 \<circ> ORD) content))::
                    TAKE (LENGTH leftover)
                      (DROP (pos + 1) (MAP (n2w:num->word8 \<circ> ORD) content)))` by fs[]
        \\ `DROP 1 (dropUntil ($= (n2w:num->word8 (ORD ch))) leftover) \<preceq>
            DROP 1 (dropUntil ($= (n2w (ORD ch)))
                (DROP pos (MAP (n2w:num->word8 \<circ> ORD) content)))` by
              (rw[] \\ Cases_on `DROP pos (MAP (n2w:num->word8 \<circ> ORD) content)`
              >-(fs[DROP_NIL])
              >-(fs[HD]
                \\ imp_res_tac chr_neq_ord_o_w8_neq
                \\ `(n2w:num->word8 \<circ> ORD) ch <> h` by metis_tac[EL_MAP]
                \\ fs[mllistTheory.dropUntil_def]
                \\ `dropUntil ($= (n2w:num->word8 (ORD ch))) (TAKE (LENGTH leftover) t) \<preceq>
         ...
         (fs[GSYM dropUntilIncl_def, GSYM LENGTH_dropUntilIncl_dropUntil]
          \\ `LENGTH
              ...
              (takeUntilIncl ($= (n2w:num->word8 (ORD ch)))
                  (DROP pos (MAP (n2w:num->word8 \<circ> ORD) content))) =
              LENGTH (DROP pos (MAP (n2w:num->word8 \<circ> ORD) content))` by
      ...
      \\ simp[] \\ rpt conj_tac
      THEN1
       (`EL pos (MAP (n2w:num->word8 o ORD) content) = HD (DROP pos (MAP (n2w \<circ> ORD) content))`
            by fs[HD_DROP]
        \\ `pos < LENGTH (MAP (n2w:num->word8 \<circ> ORD) content)` by fs[NOT_LESS, LENGTH_MAP]
        \\ `pos < LENGTH content` by fs[]
        \\ Cases_on `DROP pos (MAP (n2w:num->word8 \<circ> ORD) content)`
      ...
      THEN1
       (`EL pos (MAP (n2w:num->word8 o ORD) content) = HD (DROP pos (MAP (n2w \<circ> ORD) content))`
        ...
        \\ `t = DROP 1 (DROP pos (MAP (n2w:num->word8 \<circ> ORD) content))` by fs[]
        \\ `t = DROP (pos + 1) (MAP (n2w:num->word8 \<circ> ORD) content)` by fs[DROP_DROP, DROP_DROP_T]
        \\ qpat_x_assum `t = DROP 1 (DROP pos (MAP (n2w \<circ> ORD) content))` kall_tac
...
Theorem b_input_spec:
  ...
  NUM off offv \<and> NUM req reqv \<and>
  get_file_content fs fd = SOME(content, pos) \<Rightarrow>
  get_mode fs fd = SOME ReadMode \<Rightarrow>
...
Proof
  ...
  \\ xif
    >-(`w-r <= LENGTH bactive` by fs[LENGTH_TAKE]
      ...
      \\ ` w \<le> r + LENGTH (TAKE (w − r) (DROP r bcontent))` by fs[LENGTH_TAKE]
      ...
      >-(fs[]
        ...
        >-(xsimpl \\ simp[TAKE_TAKE, insert_atI_def, LENGTH_TAKE, TAKE_APPEND2]
          \\ `pos < LENGTH (MAP (n2w \<circ> ORD) content)` by fs[]
          \\ `LENGTH bcontent - 4 < req`
                by (fs[INT_SUB_CALCULATE, INT_ADD_CALCULATE] \\ rfs[])
          \\ `w-r <= req` by fs[]
          \\ `w-r <= LENGTH bcontent - 4` by fs[]
  ...
  >-(xapp \\ xsimpl \\ CONV_TAC(RESORT_EXISTS_CONV List.rev)
    ...
    \\ `(if
           LENGTH bcontent < 4 + (STRLEN content − pos) \<and>
    ...
    \\ `0 < STRLEN content ==> (0 < pos \<or> 0 < STRLEN content)`
        by (strip_tac \\ disj2_tac \\ fs[])
    \\ `pos = LENGTH content \<Rightarrow> MIN (LENGTH bcontent - 4 + pos) (MAX pos (LENGTH content)) = LENGTH content` by simp[MAX_DEF,MIN_DEF]
    \\ simp[MIN_DEF]
  ...
  \\ `INT (&MIN req (STRLEN content − pos + (w − r))) v_6`
      by  (pop_assum mp_tac
          ...
          \\ `req <= w + STRLEN content - pos - r` by
            (pop_assum mp_tac
             \\ qpat_x_assum `req \<le> LENGTH bcontent − 4` mp_tac
             \\ qpat_x_assum `r \<le> w` mp_tac
             \\ qpat_x_assum `r + req \<le> _` mp_tac
             \\ qpat_x_assum `4 \<le> r` mp_tac
             \\ qpat_x_assum `w \<le> LENGTH bcontent` mp_tac
  ...
  \\ qpat_x_assum `r'' <= w''` mp_tac
  \\ qpat_x_assum `req > w-r` mp_tac
  \\ qpat_x_assum `r + req \<le> w + (LENGTH bcontent − 4)` mp_tac
  \\ qpat_x_assum `req \<le> LENGTH bcontent − 4` mp_tac
...
Theorem inputLine_spec:
  INSTREAM fd fdv \<and> IS_SOME (get_file_content fs fd) \<and> get_mode fs fd = SOME ReadMode
   \<Rightarrow>
...
Proof
  ...
  xfun_spec `inputLine_aux`
    `\<forall>pp arr i arrv iv fs.
     arr \<noteq> [] \<and> i \<le> LENGTH arr \<and> LENGTH arr < arrmax \<and>
     NUM i iv \<and> pos \<le> pp \<and> pp \<le> LENGTH content \<and>
     get_file_content fs fd = SOME (content,pp) \<and>
     get_mode fs fd = SOME ReadMode \<and>
     i = pp - pos \<and>
     EVERY ($~ o $= #"\n") (TAKE i (DROP pos content)) \<and>
     i \<le> LENGTH l \<and> MAP (CHR o w2n) (TAKE i arr) = TAKE i l
     \<Rightarrow>
  ...
  >- (
    qx_gen_tac`pp` \\
    `WF (inv_image ($< LEX $<) (\<lambda>(pp,(arr:word8 list)). (arrmax - LENGTH arr, LENGTH content - pp)))`
    by (
    ...
    qabbrev_tac`P = \<lambda>(pp,arr). PC pp arr` \\
    `\<forall>x. P x` suffices_by simp[FORALL_PROD,Abbr`P`] \\
    qunabbrev_tac`PC` \\
    ...
    \\ reverse (xhandle`POSTve (\<lambda>v. &(pp < LENGTH content) * post v)
        (\<lambda>e. &(EndOfFile_exn e \<and> pp = LENGTH content)
    ...
    \\ xlet_auto >- xsimpl \\ fs [get_in_def]
    \\ `FD fd sv` by fs [FD_def]
    \\ xlet `POSTve (\<lambda>v. &(WORD ((n2w(ORD (EL pp content))):word8) v \<and>
                         pp < LENGTH content)
    ...
    >- (
      ...
      \\ `pp < LENGTH content \<Rightarrow> fd \<noteq> 1 \<and> fd \<noteq> 2`
      by (
        fs[STD_streams_def]
...
Theorem inputLines_spec:
   !fd fdv fs. INSTREAM fd fdv \<and>
   get_file_content fs fd = SOME (content,pos) \<and>
   get_mode fs fd = SOME ReadMode
   \<Rightarrow>
...
Proof
  ...
  \\ fs[strcat_thm,implode_def]
  \\ qmatch_goalsub_abbrev_tac`forwardFD fs fd n`
  \\ `n \<le> LENGTH content - pos` suffices_by (
...
Proof
  ...
  \\ reverse(xhandle`POSTve
       (\<lambda>v. &OPTION_TYPE (LIST_TYPE STRING_TYPE)
         (if inFS_fname fs f
          then SOME(all_lines fs f)
          else NONE) v * STDIO fs)
       (\<lambda>e. &(BadFileName_exn e \<and> \<not>inFS_fname fs f) * STDIO fs)`)
  ...
  \\ `\<exists>c. get_file_content fso fd = SOME (c,0)`
  ...
  \\ impl_tac >- (
    ...
    \\ `\<not>(fd = 0 \<or> fd = 1 \<or> fd = 2)` suffices_by fs[]
...
Proof
  ...
  >- (
    ...
    `\<forall>iv arr arrv. NUM 0 iv \<and> arr \<noteq> [] \<Rightarrow>
  ...
  \\ reverse (xfun_spec `inputAll_aux`
    `\<forall>i arr arrv iv fs.
     arr \<noteq> [] \<and> i \<le> LENGTH arr \<and> LENGTH arr < arrmax \<and>
     NUM i iv \<and> pos + i \<le> LENGTH content \<and>
     get_file_content fs fd = SOME (content,pos+i) \<and>
     get_mode fs fd = SOME ReadMode \<and>
  ...
  >- (
    ...
    \\ EVAL_TAC) \\
  qx_gen_tac`i` \\
  `WF (inv_image ($< LEX $<) (\<lambda>(i,(arr:word8 list)). (arrmax - LENGTH arr, LENGTH content - i)))`
  by (
    match_mp_tac WF_inv_image \\
    match_mp_tac WF_LEX \\
    simp[] ) \\
  ...
  qabbrev_tac`P = \<lambda>(i,arr). PC i arr` \\
  `\<forall>x. P x` suffices_by simp[FORALL_PROD,Abbr`P`] \\
  qunabbrev_tac`PC` \\
  match_mp_tac(MP_CANON WF_INDUCTION_THM) \\
  asm_exists_tac \\ fs[] \\
  ...
  \\ simp[LEX_DEF]
  \\ `i \<le> LENGTH content` by rw[Abbr`i`]
  \\ `j + pos < i` by rw[Abbr`i`]
  \\ `i \<le> pos + LENGTH arr` by rw[Abbr`i`]
...
Theorem print_list_spec:
   \<forall>ls lv fs out. LIST_TYPE STRING_TYPE ls lv \<Rightarrow>
...
Theorem copy_spec:
   \<forall> ino1 ino2 content1 inp out pos content2 fs inpv outv.
      ...
      pos <= STRLEN content1 \<Rightarrow>
  ...
  (xif
   ...
   >-(qexists_tac`x` >>
      ...
      `fs1 with numchars := x = fs2 with numchars := x` by (
         ...
         `ALOOKUP fs'.infds inp = SOME (ino1,ReadMode,nr + pos) \<and>
          ALOOKUP fs'.infds out = SOME (ino2,WriteMode,STRLEN content2) \<and>
          ALOOKUP fs'.inode_tbl ino1 = SOME content1 \<and>
          ALOOKUP fs'.inode_tbl ino2 = SOME content2`
...
Triviality MAP_MAP_n2w_ORD:
  (!xs. MAP (n2w \<circ> ORD) (MAP (CHR \<circ> (w2n:word8 -> num)) xs) = xs) /\
  (!xs. MAP (CHR \<circ> (w2n:word8 -> num)) (MAP (n2w \<circ> ORD) xs) = xs)
Proof
  conj_tac \\ Induct \\ fs []
QED
...
Proof
  ...
  \\ disch_then (qspecl_then [`p`,`is`,`MAP (n2w \<circ> ORD) active`] mp_tac)
...
Theorem split_exists[local]:
  !input.
    ?to_read text.
      input = to_read ++ text /\
      ((text \<noteq> "" \<Rightarrow> HD text = #"\n") \<and> EVERY (\<lambda>c. c \<noteq> #"\n") to_read)
Proof
